...
function buildTechPrompt(trendItems) {
  ...
  const quotaLine = (trendItems?.length || 0) >= 16
    ? `素材共 ${trendItems.length} 条，充足——**每个方向必须给 2 条（共 ${cats.length * 2} 条），不许少给**。\n如果找不到第二个**不重复**的真实事件，就从素材里再挑一条相关度次高的，也不要复用已用过的事件——凑数复用是严重错误。`
...
🔗 **source 字段必填**：素材里有对应事件的，必须把该素材的 url 原样填进 source（用户会点开看）。整批 16 条里 source 为空的最多 3 条（仅限素材里确实没有、且你确信真实的补充内容）。**标题也不要写成"XX新动作""XX新战略"这种占位脸——要写出具体是什么事。**
...
## 第零原则：每条都要有一个"独家梗"，别写新闻通稿
干巴巴复述新闻=失败。每条都要让程序员看完想转发到群里。

**你的梗必须来自你对这件事的独家洞察——不是套上去的模板。** 好坏对比：

❌ 差（套话，等于没说）："这项技术将彻底改变网络安全领域的格局，未来可能成为企业必备的 AI 安全员。"
✅ 好（有具体画面和吐槽）："以前黑客靠熬夜找漏洞，现在 Astra 7×24 小时不眨眼地找——安全工程师的护肝片销量怕是要涨。"

❌ 差："这一突破标志着 AI 在医疗领域的重要进展。"
✅ 好："ChatGPT 接上 Epic 病历系统，以后医生问诊的画风是：一边问你哪不舒服，一边让 AI 翻你三年前的体检报告。"

可用讲法（**每条挑不同的用，不许整批套同一个句式**）：生活化类比（把技术术语翻译成日常画面）、数字冲击（"便宜 27 倍"比"大幅降低"有劲）、反差对比（"上一代还在…现在直接…"）、程序员视角吐槽、锐评金句收尾。

禁止通稿腔："这一突破意味着""将进一步推动""迎来新的里程碑""标志着""具有重要的意义""值得关注""为…提供了新的可能性"——出现即不合格。
禁止模板复读：整批内容里如果多条都以"家人们…""这波属于是…""未来可能成为…"开头或结尾，整批作废重写。
**禁止复用同一个笑点**：多条内容都拿"XX 要失业了""这下 XX 可以躺平了"当包袱 = 偷懒，整批作废重写。每条的梗必须针对这件事本身单独想。
...
🔗 **source 字段必填**：素材里有对应事件的，必须把该素材的 url 原样填进 source（用户会点开看）。整批里 source 为空的最多 3 条。
